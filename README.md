...
- **Emotion Classification:** Convolutional Neural Network (CNN) via TensorFlow Lite <sup>[13]</sup>
...
sequenceDiagram
    ...
    participant CNN as TFLite CNN Model
    participant App as MOVA App Logic

    U->>App: Opens app
    App->>Cam: Request camera frame
    Cam->>MLKit: Pass frame (InputImage)
    MLKit->>MLKit: Detect face + landmarks
    MLKit-->>App: Face bounding box, landmark probabilities
    MLKit->>CNN: Cropped face region
    CNN->>CNN: Classify emotion (CNN inference)
    CNN-->>App: Mood label + confidence score
    App->>App: Select adaptive UI + task granularity
    App-->>U: Display mood result + tailored recommendation
```

*Figure 2. Sequence of the mood-detection pipeline, from camera capture to adaptive UI response. Face detection follows Google ML Kit's on-device processing model <sup>[12]</sup>; emotion classification follows lightweight mobile CNN architectures optimized for edge inference <sup>[13][14]</sup>.*
...
Additional third-party dependencies may carry their own licenses; consult individual dependency documentation for full terms.
